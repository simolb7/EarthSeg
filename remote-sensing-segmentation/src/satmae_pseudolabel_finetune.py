#!/usr/bin/env python
"""
Fine-tune SatMAE++ on INRIA using ground-truth labels + SAM pseudo-labels.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Loss weight for pseudo-labeled samples.",
    )
...
class InriaSemiSupervisedDataset(Dataset):
    def __init__(
        self,
        data_root: Path,
        metadata_path: Path,
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        # Train uses either GT or pseudo-label.
        # Validation always uses original ground truth.
        if self.split == "train":
            mask_path = self.data_root / row["training_mask_path"]
        else:
        ...
        # SatMAE++ FMoW-RGB uses BGR ordering.
        image = image[[2, 1, 0], :, :]
...
def build_model(args: argparse.Namespace) -> nn.Module:
    ...
    # Start again from the original SatMAE++ pretrained checkpoint.
    # Do NOT initialize from the previous fine-tuned baseline checkpoint.
...
def save_plots(
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("SatMAE + SAM pseudo-labels loss")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(
    ...
    plt.ylabel("Score")
    plt.ylim(0, 1)
    plt.title("SatMAE + SAM validation metrics")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(
        plots_dir / "metrics.png",
        dpi=180,
    )
    plt.close()
...
def main() -> None:
    ...
    if not args.pretrained_checkpoint.is_file():
        raise FileNotFoundError(
            "SatMAE++ pretrained checkpoint not found: "
    ...
    train_dataset = InriaSemiSupervisedDataset(
    ...
    val_dataset = InriaSemiSupervisedDataset(
        data_root=args.data_root,
        metadata_path=args.metadata,
        split="val",
        image_size=args.image_size,
